#ifndef RemoteProcess_HPP_
#define RemoteProcess_HPP_
...
public:
	...
	RemoteProcess( const std::string & host, const std::string & cmd, const std::chrono::duration<unsigned, std::milli> & timeout = std::chrono::duration<unsigned, std::milli>::max() )
		:
			RemoteProcess( host, split( cmd ), timeout )
	{
	}

	RemoteProcess( const std::string & host, const std::vector<std::string> & argv, const std::chrono::duration<unsigned, std::milli> & timeout = std::chrono::duration<unsigned, std::milli>::max() )
		:
			host( host ),
			argv( argv ),
...
protected:
	void init() {
		...
		if ( pid > 0 ) {
			...
			mon = std::thread( std::bind( & RemoteProcess::monitorThreadFunction, this ) );
		    return;
		}
